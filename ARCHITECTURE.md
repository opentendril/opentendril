...
*   **Unified Interface Layer (`SessionManager`):** The CLI (`tendril chat`), MCP Server (stdio/SSE), and REST API (`/v1/sessions`) all route through a unified `SessionManager`. Every interaction binds to a unique Tendril `session_id`. This allows for true **Multi-Session Independence**, where concurrent chats can operate on completely different LLM models and Epigenetic Genomes simultaneously without touching global configurations.
