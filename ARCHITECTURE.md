...
## 4. Dynamic Sequence Conductor (Sprout Loops)
...
* **Symbiotic Immune System (Destructive Fuzzing):** A `macrophage` step's genotype (`macrophage.json`) writes a native Go fuzz test (`FuzzXxx(f *testing.F)`) against the code the Worker just changed. After that Sprout turn, the Conductor — not the LLM — deterministically runs `go test -fuzz=Fuzz -fuzztime=10s` inside a dedicated, Go-toolchain-enabled, still network-isolated terrarium (`opentendril-go-fuzz:latest`; every other Go sprout keeps the minimal, toolchain-free `opentendril-go:latest`) and inspects the exit code and output for a panic or failing input. A crash is a hard, structural failure — never an LLM judgment call — that sprouts a recursive **Debugger** exactly like a Verifier failure does, so a crashing merge can't go through until the fix survives re-fuzzing.
...
## 5. Greenhouse & Daemon Persistence (the Tendril OS)

To support the **Tendril OS** (the Operating System of OpenTendril) visual Greenhouse, the Go Stem operates as a persistent, multi-session, remotely monitorable daemon.
