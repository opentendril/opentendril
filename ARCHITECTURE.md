...
*   **Symbiotic Immune System (Destructive Fuzzing, issue #154):** A `macrophage` step's genotype (`macrophage.json`) writes a native Go fuzz test (`FuzzXxx(f *testing.F)`) against the code the Worker just changed. After that agent turn, the Conductor — not the LLM — deterministically runs `go test -fuzz=Fuzz -fuzztime=10s` inside a dedicated, Go-toolchain-enabled, still network-isolated terrarium (`opentendril-go-fuzz:latest`; every other Go sprout keeps the minimal, toolchain-free `opentendril-go:latest`) and inspects the exit code and output for a panic or failing input. A crash is a hard, structural failure — never an LLM judgment call — that sprouts a recursive **Debugger** exactly like a Verifier failure does, so a crashing merge can't go through until the fix survives re-fuzzing.
