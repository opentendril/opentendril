...
**_(unset — the condition that ends this objective, stated so it can be observed
rather than judged.)_**
...
*"a Pollinator can X"*, not *"build X"*. A capability can be demonstrated; a task
can only be declared finished.

Give it a condition that someone other than the author can check. "Usage is
metered" is judged; "a run reports what it consumed, and a run that exceeds its
allowance stops and says so" is observed.
