...
// FirecrackerProvider launches sandboxes as Firecracker microVMs.
...
func (p *FirecrackerProvider) Capabilities() SandboxCapabilities {
...
func (p *FirecrackerProvider) Create(ctx context.Context, spec SandboxSpec) (Sandbox, error) {
	if ctx == nil {
	...
	cmd.Stdout = io.MultiWriter(os.Stderr, &sandbox.consoleLog)
	cmd.Stderr = io.MultiWriter(os.Stderr, &sandbox.consoleLog)
	sandbox.cmd = cmd
	...
	go func() {
		defer close(sandbox.processDone)
		_ = cmd.Wait()
	}()
	...
	if err := sandbox.waitForAgent(bootCtx); err != nil {
	...
	if spec.Timeout > 0 {
		...
		sandbox.watchdogCancel = wdCancel
		go sandbox.watchdog(wdCtx, spec.Timeout)
	}
...
// firecrackerSandbox is a live Firecracker microVM sandbox instance.
...
func (s *firecrackerSandbox) Provider() SandboxProvider {
	return s.provider
}

func (s *firecrackerSandbox) CopyIn(ctx context.Context, payloads []FilePayload) error {
	if ctx == nil {
		ctx = context.Background()
	}
...
func (s *firecrackerSandbox) Run(ctx context.Context, spec CommandSpec) (CommandResult, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	if len(spec.Command) == 0 {
		return CommandResult{}, fmt.Errorf("firecracker sandbox requires an explicit command (interactive stdin mode is not supported)")
	}
...
func (s *firecrackerSandbox) CopyOut(ctx context.Context, paths []string) ([]Artifact, error) {
	if ctx == nil {
		ctx = context.Background()
	}
...
func (s *firecrackerSandbox) SnapshotLogs(_ context.Context) (SandboxLogs, error) {
	log := s.consoleLog.String()
	return SandboxLogs{Stdout: log, Stderr: log}, nil
}
...
func (s *firecrackerSandbox) waitForAgent(ctx context.Context) error {
	for {
...
func (s *firecrackerSandbox) callAgent(ctx context.Context, req fcAgentRequest, resp *fcAgentResponse) error {
...
func (s *firecrackerSandbox) dialVsock(port uint32) (net.Conn, error) {
...
func (s *firecrackerSandbox) watchdog(ctx context.Context, timeout time.Duration) {
...
func (s *firecrackerSandbox) stopAfterTimeout() {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	_ = s.Stop(ctx)
}

func (s *firecrackerSandbox) resolveContainerPath(rawPath string) (string, error) {
...
// CheckFirecrackerReadiness verifies the host can run a Firecracker sandbox:
...
func CheckFirecrackerReadiness(ctx context.Context) error {
	...
	if strings.TrimSpace(os.Getenv("TENDRIL_FC_KERNEL_PATH")) == "" {
		return fmt.Errorf("TENDRIL_FC_KERNEL_PATH is not set")
	}
	if strings.TrimSpace(os.Getenv("TENDRIL_FC_ROOTFS_PATH")) == "" {
		return fmt.Errorf("TENDRIL_FC_ROOTFS_PATH is not set")
	}
