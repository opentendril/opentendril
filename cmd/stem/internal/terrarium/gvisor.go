...
// GVisorProvider launches terrariumes through Docker with the runsc runtime.
...
func (p *GVisorProvider) Capabilities() TerrariumCapabilities {
	return defaultTerrariumCapabilities()
}

func (p *GVisorProvider) Create(ctx context.Context, spec TerrariumSpec) (Terrarium, error) {
	return createDockerTerrarium(ctx, p, spec, "--runtime=runsc")
}
