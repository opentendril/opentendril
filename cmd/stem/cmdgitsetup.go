...
// renderGrantsYAML builds the control-plane grant authorising one delegation subject
...
// printGitSetupNextSteps prints the per-subject Model Context Protocol block and the follow-up
...
func printGitSetupUsage() {
	...
	fmt.Println("then prints the per-subject Model Context Protocol configuration. Secrets are referenced, never stored.")
	fmt.Println()
	...
	fmt.Println("  --grant-subject <s>   Also write a grant authorising delegation subject <s>")
	fmt.Println()
