...
func TestMCPModeSelection(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			os.Setenv("TERROIR_HOST", host) // Will be split but port will be lost by resolveStemAddress if it has port, wait! resolveStemAddress takes PORT.
			// Let's set TERROIR_HOST and PORT manually for the test.
			hostPart, portPart := "", ""
			...
			// Also test refusal names the variable
...
func TestMCPMode_NothingAnswers(t *testing.T) {
	// Set to an unreachable port
	os.Setenv("TERROIR_HOST", "127.0.0.1")
	os.Setenv("PORT", "65534") // Hopefully unused
	defer os.Unsetenv("TERROIR_HOST")
	defer os.Unsetenv("PORT")
