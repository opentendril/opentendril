...
* **No Unicode em dash in repository text.** Prose, comments, user-visible strings, and documentation must not use the Unicode em dash character (U+2014). Use ordinary punctuation instead: commas, colons, semicolons, parentheses, or hyphens. Source Hygiene enforces this on added lines (`scripts/check-no-em-dash.sh`, `make hygiene`, and the pre-commit hook). An em dash that already exists on the base and is left unchanged does not fail the check. The guard names the file and added line. It does not rewrite files.
