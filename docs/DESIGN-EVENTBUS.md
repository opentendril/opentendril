...
| **Task-context preparation** | `EventTaskContextAssembled` (`task-context-assembled`) — safe bounded selection provenance emitted as normal observation fan-out; raw selected evidence and private reasoning are not in the payload. |
...
- **`internal/conductor`** — primary producer: sequence complete/failure + terrarium OOM/timeout; sprout emerged/mycorrhizal-request-begun/matured/withered; stream-token, tool-invoked, sprout-transcript; task-context-assembled; parallel-sprouting / mycelial-merge; phenotypic-selection (`sequence.go`, `sprout.go`, `docker.go`, `outcome.go`, `parallelsprouting.go`, `selection.go`). The Conductor sanitizes task-context provenance before publication; EventBus remains the signalling leaf.
...
- **`health-recovered` has no in-repo publisher today.** It appears in `AllEventTypes()` (so gateway/telemetry listen for it) but healthmon never publishes it. (Four sibling reserved-but-unused types — `api-key-invalid`, `hormonal-trigger`, `rhizome-update`, `xylem-transport` — were pruned entirely rather than left as dead contract entries.)
