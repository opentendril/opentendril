...
- **`internal/dormancy`** — consumer: attaches a handler via `Subscribe` to watch for signs of life during a run's scratch test, returning the unsubscribe closure when monitoring ends. This is the bus's first **deciding** consumer (its handler's state informs orchestration decisions), whereas previous consumers were purely observers.
