...
- Guarantee delivery. Sink attachment is lossy by design (a full sink buffer drops the event for that sink only); transporter emit failures are counted and log-sampled. A bounded retry happens for transient failures, while sustained failures (or a full sink buffer) drop.
...
- **Delivery is best-effort and lossy.** Sink buffers drop events when full, and Resin is the only path that never silently drops (though it still swallows write errors). A bounded in-memory retry (3 attempts, fixed 2s backoff) covers a transient transporter failure before it's counted as a drop, but there is no ack, no durable retry queue, and no disk spool: remote sinks export data that already has a durable local copy via Resin/historydb when persistence is enabled, so a sustained-outage delivery guarantee belongs in front of the specific external platform rather than reinvented here. Note the shutdown-latency tradeoff too: a `Consume` call mid-retry can delay `bus.Shutdown()` by up to ~4s in the worst case.
- **Transporter maturity varies.** Resin and Prometheus are production-grade and directly tested (event counting, all-types pre-registration, LLM token/character counting, sprouts-active gauge, config validation, loopback bind). Webhook, Redis (raw RESP), and remote WebSocket are functional but thinner. **Kafka**: supports only a Kafka REST Proxy endpoint — a brokers-only config is rejected loudly rather than silently dropping telemetry, and does not implement native broker protocol.
