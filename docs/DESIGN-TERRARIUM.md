...
- **Provider Parity Gaps**: The four providers do not enforce identical isolation.
  - `DockerProvider` declares mounts, images, interactive I/O, and bridge/host/none network modes.
...
- **Network Posture**: `createDockerTerrarium` currently passes `--network none` unconditionally, so the declared bridge and host network modes are not applied during container creation. `FirecrackerProvider` supports only `none`. `HostProvider` is the explicit isolation escape and permits host networking.
