...
- **Session Lifecycle Adapter (`cmd/stem/internal/receptors/sessions.go`):** Implements `SessionsHandler` over `core.Core`, mapping canonical `/v1/phytomers` REST endpoints (create, list, get, update, delete, history, continue) and legacy `/v1/sessions` aliases to Core session capabilities, while supporting async sequence triggers (`runSequenceAsync`) and the event/run/current-state observation views, which sit outside the parity registry and are gated per request by `WatchAuthority` against the phytomer named in the path. `GET /v1/phytomers/{sessionId}/watch` is SSE presentation over Core `ObservePhytomer`; it does not select safe fields itself.
...
- **MCP Protocol Surface (`cmd/stem/internal/receptors/mcp.go` (protocol dispatch), `mcptools.go` (tool projection, deprecated aliases, and locked views), `mcpnames.go` (identifier projection), and `mcpgenotypeindex.go` (genotype index maintenance)):** Implements `MCPHandler`, serving stdio/JSON-RPC protocol initializations (`initialize`, `resources/list`, `resources/read`, `tools/list`, `tools/call`), projecting each Core capability as one primary lower-camelCase MCP identifier (`git.status` → `gitStatus`) plus eight deprecated compatibility aliases (`runSequence`, `sproutTendril`, `createGenotype`, `viewGenome`, `reduceGenome`, `injectPlasmid`, `graftSubstrate`, `promotePR`) and the locked MCP view `sproutWatch`. Governed names resolve to canonical Core identity before authorization or invocation. `sproutWatch` is a headless current-state snapshot of Core `ObservePhytomer`, authorized by the shared `WatchAuthority` against grant operation-class `sprout.watch`; it is not a governed command and is not counted in command parity. MCP adapters do not own continuation business logic. The handler also maintains local genotype index files (`index.yaml`) and enforces bind-time Pollen delegation authorization (`authorizeDelegatedTool`) against canonical operation-classes.
...
| `(*MCPHandler).WithSessions` / `WithDefaultSession` / `WithCore` / `WithDelegation` / `WithWatch` | Builder methods binding session state, default stdio session, Core service, delegation gate, and the shared `WatchAuthority` used by `sproutWatch`. |
...
Accepted `tools/call` families are exact-match only: a locked MCP view, a compatibility alias, the primary lower-camelCase projection, or the canonical dotted capability identifier. Unknown, hyphenated, underscored, and case-folded names fail closed. Compatibility aliases carry no independent authority and do not widen a grant. The eight aliases remain supported for existing clients. `sproutWatch` is resolved through `ResolveMCPViewToolName`, not `ResolveMCPToolName`.
