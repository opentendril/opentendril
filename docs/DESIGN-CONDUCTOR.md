...
| `DESIGN-STATELESS-SPROUT` | Sprout execution & isolation | **Matches** host ReAct + stateless terrarium tool protocol; polyglot images. Full client/image matrix remains co-owned with later `roots/llm` / sprout image audit. | **no** — shared with `roots/llm`, retire when that audit lands |
| `DESIGN-COST-OPTIMIZATION` | Dynamic orchestration (conductor-side tiering/routing) | **Matches** tier assessment and routing hooks in conductor; **default model tables and provider resolution** remain authoritative in `roots/llm`. | **no** — shared with `roots/llm`, retire when that audit lands |
