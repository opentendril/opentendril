...
`cmd/stem/internal/conductor` is the **aggregation layer** that turns a Sequence spec into executed, isolated, git-committed sprout work. It owns the Vascular Cambium (DAG sequence runner with recovery), polyglot sprout growth inside terraria, parallel speculative execution and phenotypic fitness selection, substrate configuration and credential resolution, git state externalization (commit / push / PR / branch lifecycle), genomic evolution and plasmid injection, epigenetic chronicling, and dynamic orchestration helpers (model routing, meristem planning, recursive debugger budding). It is a broad multi-subsystem package (~38 non-test source files, on the order of ~13k lines and ~365 exported symbols) driven by CLI verbs and receptors; it is not a leaf.

## Responsibilities

**Does:**

- **Sequence execution & recovery** — Load, save, list, and run YAML Sequences as a concurrency-limited DAG; support `halt` / `retry` / `pause` failure modes; bud recursive Debugger steps on verifier and macrophage failures; append meristem-produced dynamic steps; run command-only verifier steps without an LLM (`sequence.go` for the scheduler/dispatch core, `sequencepersistence.go` for YAML schema/persistence/path resolution, `sequencegenotype.go` for genotype inference and LLM-tier selection, `orchestrator.go`, `meristem.go`, `adaptation.go` for history-driven genome work, `reclaim.go` for owned isolation-branch cleanup after runs).
...
- **Budgets and lifecycle** — The growth budget is configurable and derived, not a constant. Detach replaces kill: an expired growth budget means the Stem stops waiting; the Sprout keeps growing and its terminal event arrives later (`sprout-detached` is not terminal). The **orphan reaper** provides a much longer backstop that stops a container nothing is waiting on (answering "is anyone still waiting?", never "is this going well?").
- **Post-mortem execution** — A budget bounds the work, never the account of the work. Evidence gathering, classification, and the status record run on their own bounded clock so a spent budget cannot erase what the run did.
- **Sprout image provenance** — Ensures the image a Sprout runs in exists, building it when absent from **inputs embedded in the Stem binary** rather than from a source checkout discovered at runtime (`docker.go`). An installed Stem therefore builds its images from what it carries, so provenance does not depend on where the binary was compiled or on which directories happen to be readable beside it. The `terrarium` package runs containers; it does not build them.
- **Substrate configuration** — Parse and resolve named substrates, credential profiles, auth/sign/identity/checkout/commit modes from `substrates.yaml` (`substratesconfig.go`). `patience` has three keys now — `growth`, `reap`, `scratch` — all read through one parser and all equally strict.
...
- **Chronicler & outcomes** — Distill successful run diffs into epigenetic genome notes; auto-reduce and optionally auto-push genome updates; classify sprout endings (`complete`, `no-changes`, `failed`, `timed-out` (a clock ended the run — either the terrarium watchdog or an inherited deadline; a spent growth budget now detaches instead), `skipped`, `no-engagement`, `detached`, `reaped`) and publish lifecycle events (`chronicler.go`, `outcome.go`).
...
| Chronicler & outcomes | `EpigeneticChronicler`, `NewEpigeneticChronicler`, `(*EpigeneticChronicler).TranscribeLearnings`, `(*EpigeneticChronicler).ReduceGenomeFile`, `GenomicFitness`, `SproutRunReport`, outcome constants (`SproutOutcomeComplete`, `…NoChanges`, `…Failed`, `…TimedOut`, `…Skipped`, `…NoEngagement`, `…Detached`, `…Reaped`), `ErrSproutTimedOut` |
...
- **`roots/llm`** — Every post-construction call site (`sprout.go`'s ReAct loop, `assessor.go`, `chronicler.go`, `adaptation.go`, `parallelsprouting.go`) calls through the package-internal `llmCaller` port (`Call` / `CallStream` / `CallPrompt`), not a concrete `*llm.Client`. Construction still reaches into `roots/llm` directly and is not behind a port: `docker.go`'s `resolveLLMClient`, `chronicler.go`'s `NewEpigeneticChronicler` and its genome-evolution tier-fallback loop, `assessor.go`'s per-call-site client construction, and `adaptation.go`'s coordinator resolution each call a `roots/llm` constructor (`llm.NewClientFromEnv`, `llm.NewClientForTier`, `llm.NewCoordinatorClientFromEnv`) directly, several of them behind test seams rather than one shared construction path. This remains a structural coupling: orchestration still depends on the concrete LLM package for construction, just not for usage.
...
**Structural — multi-responsibility mega-package.** Conductor spans orchestration, git, terrarium isolation, credentials, selection, chronicling, genomics, and model routing in one Go package. It is not a single clear responsibility and is a first-class candidate for a future split (sequence runner, sprout runtime, git/credentials, genomics/chronicler). File gravity reinforces this: `sequence.go` (DAG scheduling, failure policy, meristem dynamic-step application, recursive debugger budding, and dispatch) is still ~1.5k lines even after extracting its YAML schema/persistence (`sequencepersistence.go`, ~560 lines) and genotype inference (`sequencegenotype.go`, ~150 lines) into their own files; `docker.go` ~1.7k; together they dominate the package.

**Sequence runner complexity** (`sequence.go`, partially addressed). YAML schema, persistence, and path resolution (`Sequence`/`SequenceStep`/`SelectionConfig`, `LoadSequence`/`SaveSequence`, `ResolveSequencePath`, `ListSequenceFiles`, normalization) now live in `sequencepersistence.go`, and genotype inference / LLM-tier selection (`stepGenotype`, `resolveStepLLMSelection`, `inferSelectionTier`, and related helpers) now live in `sequencegenotype.go`. Additionally, `completeStep` (`sequencecompletion.go`) and `decideFailureAction` (`sequencefailurepolicy.go`) now isolate the step-completion transition and the failure-mode decision respectively. These extractions were pure, self-contained moves that made `run()` directly shorter and easier to review. However, the **remaining** concentration — DAG scheduling proper, meristem dynamic-step application (`appendDynamicSteps`), recursive-debugger budding, and the dispatch family — is unchanged and still lives in `run()`/`sequence.go`. This remaining logic reads and writes the same live mutable `sequenceRunner` state (`ready`, `queued`, `remainingDeps`, `dependents`, `retriesLeft`), and is deliberately not split further to avoid a preemptive state-machine redesign (e.g. exposing internals through a new interface or threading mutations back through return values) until a concrete need drives it.
...
**Direct `roots/llm` coupling at construction time.** Every post-construction call site is typed against the package-internal `llmCaller` port (`Call` / `CallStream` / `CallPrompt`) rather than a concrete `*llm.Client` — `chronicler.go`'s `client` field and `assessor.go`'s per-call-site clients are seamed the same way `parallelsprouting.go` and `adaptation.go` already were. Construction itself is not behind that port: `docker.go`'s `resolveLLMClient`, `chronicler.go`'s `NewEpigeneticChronicler` and genome-evolution loop, `assessor.go`'s complexity/router client seams, and `adaptation.go`'s coordinator resolution each still call a `roots/llm` constructor directly. Cost-tier routing and coordinator resolution remain entangled with sprout/sequence code paths at that construction layer (conductor-side of cost optimization lives here; client defaults live in `roots/llm`).
...
**Sequence execution & Vascular Cambium.** Sequences are DAGs with `dependsOn` (plus legacy `depends_on` normalized away), `concurrencyLimit`, and `onFailure` ∈ {halt, retry, pause}. Parallel ready steps share a worker pool; when `concurrencyLimit > 1`, individual steps also run in shadow worktrees and merge back via phloem-style `--no-ff` merge with abort-on-conflict (`runParallelSequenceStep`). Pause mode is interactive on a TTY and poll-based when headless. Status is written back into the sequence YAML so runs can resume. While paused headless, the runner honours exactly two edits to the YAML file: the sequence's `onFailure` mode and the paused step's `status`. Everything else in the file is silently ignored and overwritten by the runner's in-memory copy on the next save; the runner emits a warning to stderr when it sees such a discarded edit. Meristem steps may emit JSON step lists that `appendDynamicSteps` validates (ids, transcripts, dependency existence and acyclicity) and re-indexes into the live DAG; a rejected batch fails the run rather than being skipped. Verifier- and macrophage-named steps that fail (non-command steps) may bud a recursive Debugger step up to a depth limit, then re-queue the failed step — self-healing without a separate control plane.
...
**Substrate configuration.** Named substrates carry URL/path, branch, auth, signing, identity, checkout mode, commit mode, default-branch protection (opt-out), and optional credential profiles. Secrets are references (env names, key paths), never values in YAML. Read-only substrates allow local edits inside the terrarium but block merge-back. When an execution lacks an explicit terrarium provider choice, the conductor automatically prefers gVisor if the host supports it (via the `runsc` Docker runtime), falling back to Docker. Explicit provider selections (via `TENDRIL_TERRARIUM_PROVIDER` or an explicit substrate value like `docker`) are always honored verbatim and never overridden.
