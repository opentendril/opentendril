...
**Sequence runner complexity** (`sequence.go`, partially addressed). YAML schema, persistence, and path resolution (`Sequence`/`SequenceStep`/`SelectionConfig`, `LoadSequence`/`SaveSequence`, `ResolveSequencePath`, `ListSequenceFiles`, normalization) now live in `sequencepersistence.go`, and genotype inference / LLM-tier selection (`stepGenotype`, `resolveStepLLMSelection`, `inferSelectionTier`, and related helpers) now live in `sequencegenotype.go` — both were pure, self-contained code with no shared mutable state, so the move was a same-package file split with no behavior change. Still concentrated in `sequence.go`, and **deliberately not split further**: DAG scheduling, halt/retry/pause failure policy, and meristem dynamic-step application (`appendDynamicSteps`) all read and write the same live `sequenceRunner` state (`ready`, `queued`, `remainingDeps`, `dependents`, `retriesLeft`) inside `run()`, and recursive-debugger budding shares that same state. Pulling those apart would require exposing the scheduler's internals through a new interface or threading mutations back through return values — a real state-machine redesign, not a mechanical move — so it's deferred until a concrete need (e.g. a bug that's hard to fix because of this concentration) drives it, rather than decomposed preemptively.
