...
There is no legacy plan-doc for this package. `docs/DESIGN-VASCULAR-CAMBIUM.md` describes Conductor parallel-step (vascular cambium) execution and is not a gateway design. Where any checklist or narrative claimed otherwise, the shipped code in `gateway.go` / `gateway_test.go` and the serve wiring in `cmdserve.go` are authoritative.
