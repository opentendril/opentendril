...
- Task-context preparation may inspect only the canonical backing/source repository's local `.tendril/rhizome.db` with its matching local key for project memory. A memory is admitted to automatic task context only when its `status` is `established` and its `authority` is `botanist` or `deterministic`; all other status/authority combinations are omitted with a granular omission reason (`memory-unclassified`, `memory-proposed`, `memory-stale`, `memory-conflicted`, `memory-rejected`, `memory-superseded`). Evidence-bound memories (those carrying a v1 `EvidenceManifest` in `RevisionMetadata`) are additionally validated by recomputing the SHA-256 content hash of each referenced workspace file; a hash mismatch is classified as stale. When the execution workspace is the canonical source repository, a stale transition is persisted to the source-local Rhizome index. Configured Pinecone, Weaviate, and arbitrary shared SQLite backends remain outside automatic task-context sourcing.
- **Remote backend GetMemory uses exact fetch by deterministic ID.** `PineconeMemoryBackend.GetMemory` fetches by `memoryID(repositoryName, title)` (a SHA-256-prefixed ID); `WeaviateMemoryBackend.GetMemory` fetches by `deterministicUUID(repositoryName, title)`. Both fail closed on repository/title mismatch, malformed envelope, or conflicting stored StableID. HTTP 404 from Weaviate returns `found=false`; empty `vectors` from Pinecone returns `found=false`. Neither uses list or search to establish exact identity in the lifecycle path. Coverage includes SQLite GetMemory, Pinecone GetMemory (httptest), Weaviate GetMemory (httptest), Go/regex/tree-sitter parsers, crypto round-trip, and envelope serialization logic.
...
**Evidence and Git Substrate root.** Evidence paths are evaluated against the canonical Git worktree root resolved by `git rev-parse --show-toplevel`, which returns the same root regardless of which subdirectory the command is executed from. This root is resolved by `currentSubstrateRoot()` at the CLI boundary. Path validation explicitly rejects `..` traversals and absolute paths prior to canonicalization.

**Durable memory identity.** `StableMemoryIdentity(repositoryName, title)` computes the SHA-256 hex of the byte sequence `repositoryName + NUL + title`. This function is a durable, backend-independent identity primitive and does not trim, normalize, or modify its inputs. Whitespace and null bytes in the inputs are part of the identity. Botanist lifecycle operations (add, supersede) normalize their caller-supplied title inputs at the policy boundary -- for example, `ApplyBotanistAdd` trims the supplied title before constructing the Memory -- so that the resulting canonical title drives the identity. This means that a replacement title `" Rule "` normalizes to `"Rule"` before identity construction, producing the same StableID as the old title `"Rule"` and causing supersession to fail before any backend write.

**Collision-safe add.** `ExecuteBotanistAdd` calls `GetMemory` to prove absence before storing. If an exact identity collision is detected, the add is rejected without any store write.

**Collision-safe supersession.** `ApplySupersede` enforces: (1) replacement title normalization produces a StableID that must differ from the old StableID; (2) `GetMemory` confirms the replacement identity does not already exist; (3) the old record is marked superseded before the replacement is stored; (4) if the old-write fails, the replacement is aborted; (5) if the replacement write fails after the old is marked superseded, the old stays superseded and must be re-submitted manually.

**GetMemory authority.** `backend.GetMemory` is the lifecycle authority for confirming target existence, rejecting collision, and verifying absence before replacement. Bounded `ListMemories` and `SearchMemories` are not used to establish exact identity in the lifecycle path.

**Supersession.** Knowledge evolution uses a fail-closed chronological supersede mechanism instead of direct modification. Supersession enforces strict ordering: the old item is marked superseded before the correction replacement is persisted. If the old record fails to update, the new record is aborted, preventing old and new from existing simultaneously as equivalent established truths. If the replacement write fails after the old is marked superseded, the old remains superseded; the caller must re-submit the replacement manually.

`GenerateMemoryMap` renders search hits as a category-sectioned Markdown Memory Map. To prevent the Botanist from mistaking unclassified or proposed content for established knowledge, it visually highlights non-established memory status. It remains an available Rhizome rendering function, but the Conductor no longer stages that map as automatic Sprout context. Task-context selection belongs to the Conductor, not Rhizome. Automatic Mycorrhizal task context is now limited to memories that pass the eligibility gate (`status=established`, `authority=botanist|deterministic`) and, for evidence-bound memories, a deterministic file-hash validation against the execution workspace. Full Repo Map remains a fallback on-disk context artifact.

**Two facets, one leaf.** Structural symbols and project memory remain co-located capabilities of the Rhizome package and use the same SQLite schema and encryption machinery. For a managed Sprout growth, the Conductor may open distinct Rhizome stores for distinct purposes: the execution-workspace store supplies refreshed structural evidence, while the canonical backing/source repository’s local Rhizome store is the only project-memory store that automatic task-context preparation may inspect. Records from that store are eligible only when `status=established` and `authority=botanist|deterministic`; all other combinations are counted and omitted. Evidence-bound records additionally require SHA-256 file-hash validation. Both remain Rhizome stores; eligibility policy and task-context selection belong to the Conductor.
