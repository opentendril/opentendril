...
**Project memory (RAG-shaped backend).** Cross-session decisions and notes live beside the symbol index. `MemoryBackend` is a narrow CRUD+search port; SQLite FTS is the default local implementation sharing the same DB file and encryptor as the symbol index. Pinecone and Weaviate adapters exist for managed/off-box storage and match the historical “abstract vector store” direction, but as built they are **HTTP adapters with simplified retrieval** (hash vectors / BM25), not a full embedding pipeline. `GenerateMemoryMap` renders search hits as a category-sectioned Markdown Memory Map; it remains an available Rhizome rendering function, but the Conductor no longer stages that map as automatic Sprout context. Unclassified legacy memory is quarantined from automatic Sprout task context. Full Repo Map remains a fallback on-disk context artifact, while task-context selection belongs to the Conductor, not Rhizome.
