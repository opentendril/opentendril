...
| `SeedRun` | One `seed.grow` bounded-task record: handle, dispatching Pollen, substrate, goal, status, iterations, Fruit (branch, commit, diff, logs, error, timestamps), publication diagnostic, and verification diagnostics. |
