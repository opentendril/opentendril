...
- Persist Sprout execution history as a lifecycle upsert (once when the Sprout emerges (`running`) and again when it matures or withers), recording the dispatching Pollen, the substrate the work targeted, the resolved provider, the usage envelope, the structured observation envelope (outcome, failure category, Core-approved failure stage and diagnostic code, safe provider diagnostic, whether a Mycorrhizal request was attempted, tool invocation count, and `terrariumProvider`), and exact Fruit provenance when a reviewable commit exists (stable repository identity, private local locator, branch, commit, publication state, and creation time) so a read can be scoped to the subject that owns it and a Botanist can explain the run without parsing free-text errors (`RecordSproutRun` / `LoadSproutRuns`). The stage and code use the existing JSON observation envelope; historical envelopes that omit them load empty, and raw stored errors are never used to reconstruct them. `terrariumProvider` is the recorded provider that actually created the Sprout's Terrarium. It is stored in the existing `sproutruns.observation` JSON envelope and does not require a separate SQLite column. Historical rows may legitimately omit it.
...
- Persist `seed.grow` bounded-task runs keyed by the durable `handle` a Pollinator collects against, recording the dispatching Pollen so collection can be scoped, plus the reviewable Fruit (status, iterations, branch, diff, logs, stable repository identity, private local locator, publication state, and creation time) and the structured observation envelope (safe Fruit-publication diagnostic and bounded verification diagnostics) (`RecordSeedRun` / `GetSeedRun`).
- List structural Fruit claims across the existing Seed and Sprout execution records with `ListFruitClaims`. A claim requires an exact persisted non-empty branch and commit; this read does not infer claims from branch names, mutable Substrate aliases, terminal events, or commit messages, does not cap results by Phytomer, and does not classify forge/review state.
- Supply the durable evidence consumed by the Stem's Botanist Fruit inventory. The serving Stem and `tendril fruit list` open the same persisted store and pass `ListFruitClaims` through the package-main evidence adapter to Core. Core performs review-state classification; HistoryDB does not.
...
| `SproutRun` | One Sprout execution record (runId, session/step/origin, pollen, substrate, provider, model, genotype, transcript, status, output, error, timestamps, usage, outcome, failureCategory, failureStage, diagnosticCode, providerDiagnostic, providerRequestAttempted, toolInvocations, terrariumProvider, and persisted Fruit provenance). `terrariumProvider` is the recorded provider that actually created the Sprout's Terrarium. It is stored in the existing `sproutruns.observation` JSON envelope and does not require a separate SQLite column. Historical rows may legitimately omit it. |
| `SeedRun` | One `seed.grow` bounded-task record: handle, dispatching Pollen, substrate, goal, status, iterations, Fruit (branch, commit, stable repository identity, private local locator, publication state, creation time, diff, logs, error, timestamps), publication diagnostic, and verification diagnostics. |
...
| `ListFruitClaims` | List every persisted exact Fruit branch/commit pair from Seed and Sprout execution rows; structural evidence only, with no forge or review-state classification. |
...
| `PruneOlderThan` | Delete eligible non-Fruit rows older than a cutoff and compact payloads on Fruit-bearing rows while preserving structural Fruit provenance; no forge lookup or age-based Fruit disposal. `VACUUM` runs when rows are deleted or payloads are compacted. |
...
- **Fruit inventory requires persisted evidence.** When logging is disabled, the local Fruit view is not wired and the CLI/private REST route report unavailable rather than returning an empty success result. A HistoryDB or evidence-source failure is likewise reported as unavailable; no branch enumeration fallback exists.
