...
| `FailureCategory` / `ClassifyFailure` / `ObservationFacts` / `ProviderDiagnostic` | The Sprout-run observation contract: a closed kebab-case failure category owned by Core, classified from typed facts only (never free-text errors). Adapters persist and render the result. |
...
| `LooksLikeAccessToken` / `LooksLikePollinatorCredential` | Prefix routing (`tendril_access_` vs `tendril_root_`) so a surface sends a bearer to the right resolver. Both live under the `tendril_` namespace, and the full two-segment prefix is the discriminator — never the namespace alone; the two are mutually exclusive. |
...
- **The confirm-above-impact bound prompts rather than denies.** A confirm-above invocation creates a pending confirmation (not a denial), a Botanist approves or denies it via REST (`/v1/delegation/pending`) or the CLI (`tendril delegation pending|approve|deny`), and an approved confirmation authorizes exactly one matching retry within the pending window (1-hour TTL). A pending approval is validated against the live grant at consumption time, not a stale snapshot — so revoking or narrowing a grant while a confirmation is outstanding takes effect immediately, not just as prevention of new denials.
